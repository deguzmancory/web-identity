...
const TPAWelcome: React.FC<Props> = ({ location, onSetWelcomeScreen }) => {
  ...
  useEffect(() => {
    ...
    // Check for query params "username", and "token". Should be included in link sent to email from setup account.
    if (!query.has('username') || !query.has('token')) {
      ...
      const username = `${Tenants.ADMIN}|${email.trim().toLowerCase()}`;
...
export default connect(mapStateToProps, mapDispatchToProps)(TPAWelcome);
